...
BAD_IMAGE_TOKENS = ['head_copy', 'body_copy', 'headline', 'body', '헤드카피', '바디카피', '카피:', 'caption:', 'title:']
...
def clean(value, fallback=''):
    ...
    fixes = {'왔습니다입니다': '왔습니다', '합니다입니다': '합니다', '됩니다입니다': '됩니다', '있습니다입니다': '있습니다', '입니다입니다': '입니다', '때문입니다입니다': '때문입니다'}
...
def compact_source_excerpt(text, head=9000, tail=2500):
    text = clean(text)
...
def collect_facts(row):
    ...
    for key in ['interpretation_slots', 'cardnews_seed', 'source_index', 'interpretation_report']:
        ...
        if isinstance(obj, dict):
            for sub in ['context_profile', 'fact_roles', 'fact_seeds', 'timeline_seeds', 'slide_seed', 'source_index', 'interpretation_slots']:
                item = obj.get(sub)
                if isinstance(item, dict):
                    pool.extend(item.values())
                elif isinstance(item, list):
                    pool.extend(item)
    pool.extend(split_sentences(rv(row, 'transcript'))[:160])
    out, seen = [], set()
    for item in pool:
        text = flatten(item)
...
def keyword_hits(text, tokens):
    return [t for t in tokens if t in clean(text)]
...
def evidence_role(sentence):
    s = clean(sentence)
    if count_any(s, ['주가', '급락', '폭락', '고점', '저점', '거래정지']): return 'shock'
    if count_any(s, ['소액주주', '시총', '규모', '명', '조원', '억원']): return 'scale'
    if count_any(s, TOK['expect']): return 'hype'
    if count_any(s, TOK['speaker']): return 'speaker'
    if count_any(s, ['적자', '의견거절', '상장폐지', '상폐', '관리종목', '축소', '손실']): return 'collapse'
    if count_any(s, TOK['finance']): return 'finance'
    if count_any(s, TOK['execute']): return 'execution'
    if count_any(s, ['교훈', '확인', '체크', '기준', '질문']): return 'lesson'
    return 'general'


def build_editorial_angle_candidates(source_text, mode, evidence_bank):
    ...
    if mode == 'market_or_company_shift':
        ...
        angles.append('한 종목 사례를 통해 다음 테마주를 볼 때 확인할 기준을 정리하는 구조')
        angles.append('화려한 미래 서사와 차가운 숫자가 충돌하는 순간을 보여주는 구조')
    elif mode == 'stakeholder_conflict':
        angles = ['겉으로 보이는 갈등보다 각 이해관계자가 감수하는 비용을 보여주는 구조', '요구와 책임이 충돌하는 지점을 순서대로 추적하는 구조', '감정적 반응을 판단 기준으로 재배열하는 구조']
    elif mode == 'howto_or_warning':
        angles = ['사람들이 따라 하기 전에 놓치는 조건을 보여주는 구조', '결론보다 원리와 적용 기준을 먼저 설명하는 구조', '흔한 실수를 체크리스트로 바꾸는 구조']
    else:
        angles = ['표면적인 결과보다 그 결과를 만든 구조를 설명하는 관점', '사람들이 왜 이 메시지에 반응하는지 심리와 맥락을 풀어내는 관점', '사례 하나를 일반화 가능한 기준으로 바꾸는 관점']
    dedup = []
    for a in angles:
        if a not in dedup:
            dedup.append(a)
...
def build_card_questions(rich_pack):
    mode = rich_pack.get('content_mode', 'issue_context')
    if mode == 'market_or_company_shift':
        return ['이 사건에서 가장 충격적인 결과는 무엇인가?', '사람들은 왜 이 회사를 믿게 되었나?', '그 믿음은 어떤 서사와 스피커를 통해 커졌나?', '어떤 숫자와 사실이 그 믿음을 흔들었나?', '왜 투자자들은 빠르게 빠져나오기 어려웠나?', '다음 유사 사례에서 무엇을 먼저 확인해야 하나?']
    if mode == 'stakeholder_conflict':
...
def build_structural_insight_pack(rich_pack):
    mode = rich_pack.get('content_mode', 'issue_context')
    if mode == 'market_or_company_shift':
        return {
...
def build_rich_context_pack(row):
    ...
    joined = ' '.join([topic, rv(row, 'keywords'), rv(row, 'event_type'), source_text] + facts[:40])
    ...
    for s in facts + sentences:
        ...
        if len(evidence_bank) >= 30:
            break
    angles = build_editorial_angle_candidates(source_text, mode, evidence_bank)
    rich = {
        'source_title': clean(topic), 'source_length': len(source_text), 'source_excerpt': compact_source_excerpt(source_text),
        'subject': subject, 'content_mode': mode, 'numbers': extract_numbers(source_text)[:14],
        'keywords': {'market': keyword_hits(source_text, TOK['market']), 'hype': keyword_hits(source_text, TOK['expect']), 'risk': keyword_hits(source_text, TOK['conflict'] + TOK['verify']), 'speaker': keyword_hits(source_text, TOK['speaker'])},
        ...
        'editorial_angle_candidates': angles, 'chosen_angle': angles[0] if angles else '핵심 사건의 구조와 교훈',
        'narrative_arc': build_narrative_arc(mode), 'card_role_questions': [], 'evidence_bank': evidence_bank,
        'tone_dna': {'style': '강한 오프닝, 맥락 설명, 숫자/사실 근거, 마지막 교훈', 'emotion_curve': '충격 → 이해 → 의심 → 붕괴 → 교훈', 'avoid': ['과도하게 추상적인 문장', '같은 표현 반복', '내부 라벨형 카피', '증권 추천처럼 보이는 문장']},
    }
    rich['card_role_questions'] = build_card_questions(rich)
    rich['structural_insight_pack'] = build_structural_insight_pack(rich)
    return rich
...
def default_cardnews(pack):
    ...
    return {'title': f"{pack['subject']} 카드뉴스", 'core_problem': pack['audience_problem'], 'main_message': pack['chosen_angle'], 'cta': '다음 유사 사례를 볼 때, 무엇을 먼저 확인해야 할까요?', 'cards': [
...
def default_shorts(pack):
    ...
    return {'title': f"{pack['subject']} 1분 쇼츠", 'hook': f"{nums} 이 숫자만 보면 놓치는 게 있습니다.", 'script': f"""0초
{nums}
이건 단순한 결과가 아닙니다.
...
55초
이건 기회일까요, 경고일까요?
댓글로 여러분의 판단을 남겨주세요.""", 'shot_plan': [], 'cta': '댓글로 여러분의 판단을 남겨주세요.'}


def default_midform(pack):
    nums = ', '.join(pack.get('numbers', [])[:4]) or pack.get('key_metric', '')
    return {'title': f"{pack['subject']} 5분 미드폼 대본", 'hook': f"{nums} 이 숫자의 의미를 다시 봐야 합니다.", 'script': f"""오프닝
{nums}
...
근거
여기서 봐야 할 첫 번째 근거는 {shorten(pack['verification'], 220)}입니다. 기대가 커질수록 시장은 더 구체적인 증거를 요구합니다. 말보다 숫자, 분위기보다 실적, 스토리보다 진행 상황을 보게 됩니다.

리스크
두 번째는 자금과 실행 리스크입니다. {shorten(pack['finance'], 180)} 그리고 {shorten(pack['execution'], 180)} 이런 요소들이 겹치면 시장은 더 이상 같은 기준으로 보지 않습니다.

핵심 판단
결국 핵심은 이겁니다. {shorten(pack['tension'], 220)} 이 하락이 과한 공포인지, 뒤늦은 재평가인지 보려면 가격보다 먼저 기대가 아직 숫자로 증명되고 있는지 확인해야 합니다.

마무리
그래서 이건 단순히 떨어졌냐, 싸졌냐의 문제가 아닙니다. 다시 볼 기회인지, 아직 남은 경고인지 판단하려면 차트보다 먼저 근거를 봐야 합니다. 여러분은 이 흐름을 어떻게 보시나요?""", 'chapter_plan': [], 'cta': '여러분은 이 흐름을 어떻게 보시나요?'}


def build_default(output_type, pack):
    if output_type == '카드뉴스 6장': return default_cardnews(pack)
    if output_type == '1분 쇼츠 대본': return default_shorts(pack)
    return default_midform(pack)


def build_quality_rubric(pack):
    return {'min_numeric_mentions': 2 if pack.get('content_mode') == 'market_or_company_shift' else 1, 'min_cards': 6, 'min_body_length_card2_3': 70, 'forbidden_phrases': ['현실 신호', '핵심 포인트', '확인해보세요', '기준은 저장해두고 다시 확인해보세요'] + BAD_LABELS}
...
def validate_cardnews_quality(cards, pack):
    ...
    for idx, card in enumerate(cards[:6], start=1):
        ...
        heads.append(head)
        if not head: issues.append(f'{idx}장 헤드카피 누락')
        if not body: issues.append(f'{idx}장 바디카피 누락')
        if idx in [2, 3] and len(body) < rubric['min_body_length_card2_3']: issues.append(f'{idx}장 설명 밀도 부족')
        numeric_count += len(extract_numbers(head + ' ' + body))
        for bad in rubric['forbidden_phrases']:
            if bad and (bad in head or bad in body): issues.append(f'{idx}장 금지 표현 포함: {bad}')
    if len(set(heads)) < max(4, len(heads) - 1): issues.append('헤드카피 중복률이 높음')
    if numeric_count < rubric['min_numeric_mentions']: issues.append('구체 숫자 반영 부족')
    last = clean(cards[5].get('body') or cards[5].get('body_copy') or '') + ' ' + clean(cards[5].get('headline') or cards[5].get('head_copy') or '')
    if not any(k in last for k in ['질문', '확인', '기준', '체크', '교훈', '다음', '숫자']): issues.append('마지막 장에 교훈/체크 질문 부족')
...
- 각 장마다 evidence_bank의 구체 근거를 최소 하나 반영하라.
...
def call_llm(provider, key, model, prompt, temperature):
    ...
    if provider == 'OpenRouter': headers.update({'HTTP-Referer': 'https://eafi-benchmark-db.streamlit.app', 'X-Title': 'EAFI Content Generator'})
...
def normalize_card(src, base):
    return {'headline': clean(src.get('headline') or src.get('head_copy') or base.get('headline') or base.get('head_copy') or ''), 'body': clean(src.get('body') or src.get('body_copy') or base.get('body') or base.get('body_copy') or ''), 'scene': sanitize_image_description(src.get('scene') or src.get('image_description') or base.get('scene') or base.get('image_description') or '')}
...
def validate_result(output_type, obj, default_output, pack=None):
    ...
    if output_type == '카드뉴스 6장':
        ...
        result = {'title': clean(obj.get('title'), default_output['title']), 'core_problem': clean(obj.get('core_problem'), default_output['core_problem']), 'main_message': clean(obj.get('main_message'), default_output['main_message']), 'cta': clean(obj.get('cta'), default_output['cta']), 'cards': fixed}
        if pack: result['quality'] = validate_cardnews_quality(fixed, pack)
        return result
    if output_type == '1분 쇼츠 대본':
...
def main():
    ...
    st.caption('rich_context_pack 기반으로 카드뉴스, 1분 쇼츠, 5분 미드폼을 선택 생성합니다.')
    ...
    state_key = f"rich_multi_output_{output_type}_{int(row['id'])}_{provider}_{model}"
    if run_rule or run_llm or state_key not in st.session_state:
        ...
        if run_llm and provider != '규칙 기반만 사용' and api_key and model:
            try:
                ...
                if status == 429: st.error('LLM 호출 제한에 걸렸습니다. 잠시 후 다시 시도하거나 다른 Provider로 바꿔보세요. API 키는 화면에 표시하지 않습니다.')
                else: st.error(f'LLM 호출 실패: HTTP {status}. 규칙 기반 초안으로 대체합니다.')
    ...
    if output_type == '카드뉴스 6장':
        ...
        if quality.get('passed'): st.success('품질 검사 통과')
        else: st.warning('품질 검사 이슈: ' + ' / '.join(quality.get('issues', [])))
        edited_cards = []
        for i, card in enumerate(result['cards'], 1):
            with st.container(border=True):
                ...
                body = st.text_area('바디카피', value=card['body'], height=120, key=f'{state_key}_b_{i}')
