...
CONTENT_ANGLE_PRESETS = {
    "서론 훅 기준 자동 생성": "__INTRO_HOOK__",
    "벤치마크 적용 아이디어 사용": "__REFERENCE_APPLICATION__",
    "벤치마크 제목 사용": "__REFERENCE_TITLE__",
    "기업 영상이 문의로 이어지지 않는 이유": "기업 영상이 문의로 이어지지 않는 이유",
    "예쁜 영상인데 전환이 약한 이유": "예쁜 영상인데 전환이 약한 이유",
    "제작비가 조용히 새는 지점": "영상 제작비가 조용히 새는 지점",
    "레퍼런스를 따라 해도 안 먹히는 이유": "레퍼런스를 따라 해도 우리 영상이 안 먹히는 이유",
    "좋은 영상과 전환되는 영상의 차이": "보기 좋은 영상과 문의로 이어지는 영상의 차이",
    "브랜드 영상 제작 전 체크해야 할 4가지": "브랜드 영상 제작 전 반드시 정리해야 할 4가지",
    "유튜브 콘텐츠가 브랜드 자산이 되지 않는 이유": "유튜브 콘텐츠가 브랜드 자산으로 쌓이지 않는 이유",
    "AI 영상 제작을 맡기기 전 봐야 할 것": "AI 영상 제작을 맡기기 전에 먼저 봐야 할 것",
    "직접 입력": "직접 입력",
}
...
def select_intro_frame():
    ...
    selected = st.selectbox("서론 훅 관점", labels, index=1, key="cr4_intro_frame")
    preset = dict(INTRO_HOOK_PRESETS[selected])
    if selected == "직접 입력":
        preset["hook"] = st.text_area("서론 훅 직접 입력", value="", height=70, key="cr4_intro_hook")
        preset["body"] = st.text_area("서론 보조 문장 직접 입력", value="", height=90, key="cr4_intro_body")
        preset["problem"] = st.text_area("서론 기반 핵심 문제 직접 입력", value="", height=90, key="cr4_intro_problem")
    return selected, preset


def select_content_angle(row, intro):
    labels = list(CONTENT_ANGLE_PRESETS.keys())
    selected = st.selectbox("이번 카드뉴스의 핵심 각도", labels, index=0, key="cr4_content_angle_select")
    value = CONTENT_ANGLE_PRESETS[selected]
    if value == "__INTRO_HOOK__":
        return intro["hook"].replace("\n", " ").strip(), selected
    if value == "__REFERENCE_APPLICATION__":
        return clean(row.get("eafi_application"), clean(row.get("title"), intro["hook"].replace("\n", " "))), selected
    if value == "__REFERENCE_TITLE__":
        return clean(row.get("title"), intro["hook"].replace("\n", " ")), selected
    if selected == "직접 입력":
        return st.text_area("핵심 각도 직접 입력", value="", height=80, key="cr4_content_angle_custom"), selected
    return value, selected


def select_core_problem(row, intro):
    labels = list(CORE_PROBLEM_PRESETS.keys())
    selected = st.selectbox("핵심 문제", labels, index=0, key="cr4_core_problem_select")
    ...
    if selected == "직접 입력":
        return st.text_area("핵심 문제 직접 입력", value="", height=90, key="cr4_core_problem_custom"), selected
    return value, selected
...
def update_draft_state(plan, signature, force=False):
    if force or st.session_state.get("copy_reviewed_v4_signature") != signature or "copy_reviewed_v4_plan" not in st.session_state:
        st.session_state["copy_reviewed_v4_signature"] = signature
        st.session_state["copy_reviewed_v4_plan"] = plan
        st.session_state["copy_reviewed_v4_version"] = st.session_state.get("copy_reviewed_v4_version", 0) + 1


def render_editable_plan(plan, version):
    suffix = f"v{version}"
    st.markdown("### 핵심 설계")
    title = st.text_input("저장할 주제명", value=plan["title"], key=f"cr4_title_{suffix}")
    core_problem = st.text_area("핵심 문제", value=plan["core_problem"], height=80, key=f"cr4_problem_{suffix}")
    main_message = st.text_area("메인 메시지", value=plan["main_message"], height=90, key=f"cr4_message_{suffix}")
    cta = st.text_input("CTA", value=plan["cta"], key=f"cr4_cta_{suffix}")
    ...
    for idx, (copy, image) in enumerate(zip(plan["slides"], plan["images"]), start=1):
        with st.container(border=True):
            st.markdown(f"#### {idx}장")
            slides.append(st.text_area("카피", value=copy, height=130, key=f"cr4_slide_{idx}_{suffix}"))
            images.append(st.text_area("이미지 방향", value=image, height=110, key=f"cr4_image_{idx}_{suffix}"))
...
def main():
    st.set_page_config(page_title="Cardnews Planner Copy Reviewed V4", page_icon="🧩", layout="wide")
    init_cardnews_table()

    st.title("🧩 카드뉴스 설계안 생성 Copy Reviewed V4")
    st.caption("핵심 각도까지 드롭다운으로 선택하는 버전입니다.")
    ...
    with c4:
        ...
        target, target_label = select_with_custom("타깃 고객", TARGET_PRESETS, "cr4_target", "브랜드/마케팅 담당자")
        cta, cta_label = select_with_custom("CTA", CTA_PRESETS, "cr4_cta_preset", "DM 포트폴리오/견적")
        angle, angle_label = select_content_angle(row, intro)
        problem, problem_label = select_core_problem(row, intro)
    with c5:
        st.markdown("#### 선택된 핵심 각도 미리보기")
        st.write(angle)
    ...
    p1, p2 = st.columns(2)
    with p1:
        insight, insight_label = select_with_custom("벤치마크에서 가져올 인사이트 / 전개 구조", INSIGHT_PRESETS, "cr4_insight", "목적-타깃-메시지-CTA", height=90)
        proof, proof_label = select_with_custom("근거 / 설명", PROOF_PRESETS, "cr4_proof", "수정 리스크 감소", height=90)
    with p2:
        solution, solution_label = select_with_custom("브랜드 관점의 해결 방식", SOLUTION_PRESETS, "cr4_solution", "전환 구조 먼저 설계", height=90)
        offer, offer_label = select_with_custom("서비스 / 제안 문장", OFFER_PRESETS, "cr4_offer", "브랜드/제품/유튜브 통합", height=90)
    ...
    signature = "|".join([
        ...
        brand, target_label, cta_label, angle_label, problem_label, insight_label, solution_label, proof_label, offer_label,
        target, cta, angle, problem, insight, solution, proof, offer,
    ])
    ...
    with col_refresh:
        if st.button("현재 선택값으로 초안 다시 생성"):
            ...
            st.rerun()
    with col_info:
        st.info("핵심 각도와 핵심 문제를 모두 드롭다운으로 선택할 수 있습니다. 선택값이 바뀌면 하단 초안이 자동 갱신됩니다.")

    st.markdown("---")
    draft = st.session_state.get("copy_reviewed_v4_plan", plan)
    version = st.session_state.get("copy_reviewed_v4_version", 0)
