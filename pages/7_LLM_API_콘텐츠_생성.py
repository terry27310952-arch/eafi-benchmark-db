...
def validate_result(output_type, obj, default_output):
    ...
    if output_type == '카드뉴스 6장':
        ...
        for i in range(6):
            ...
            fixed.append({
                'headline': clean(src.get('headline') or base['headline']),
                'body': clean(src.get('body') or base['body']),
                'scene': clean(src.get('scene') or base['scene']),
            })
        return {
...
def main():
    ...
    st.caption('한 번 분석한 원본으로 카드뉴스, 1분 쇼츠, 5분 미드폼 대본을 선택 생성합니다.')
    ...
    state_key = f"multi_output_{output_type}_{int(row['id'])}"
    if run_rule or run_llm or state_key not in st.session_state:
        ...
        if run_llm and provider != '규칙 기반만 사용' and api_key and model:
            ...
            try:
                ...
                if status == 429:
                    st.error('LLM 호출 제한에 걸렸습니다. 잠시 후 다시 시도하거나 Groq/OpenRouter로 바꿔보세요. API 키는 화면에 표시하지 않습니다.')
                else:
