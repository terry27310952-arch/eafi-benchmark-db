...
def select_intro_frame():
    ...
    selected = st.selectbox("서론 훅 관점", labels, index=1, key="copy_reviewed_intro_frame")
    preset = dict(INTRO_HOOK_PRESETS[selected])
    if selected == "직접 입력":
        preset["hook"] = st.text_area("서론 훅 직접 입력", value="", height=70, key="copy_reviewed_intro_hook")
        preset["body"] = st.text_area("서론 보조 문장 직접 입력", value="", height=90, key="copy_reviewed_intro_body")
        preset["problem"] = st.text_area("서론 기반 핵심 문제 직접 입력", value="", height=90, key="copy_reviewed_intro_problem")
...
def update_draft_state(plan, signature, force=False):
    if force or st.session_state.get("copy_reviewed_signature") != signature or "copy_reviewed_plan" not in st.session_state:
        st.session_state["copy_reviewed_signature"] = signature
        st.session_state["copy_reviewed_plan"] = plan
        st.session_state["copy_reviewed_version"] = st.session_state.get("copy_reviewed_version", 0) + 1


def render_editable_plan(plan, version):
    suffix = f"v{version}"
    st.markdown("### 핵심 설계")
    title = st.text_input("저장할 주제명", value=plan["title"], key=f"cr_title_{suffix}")
    core_problem = st.text_area("핵심 문제", value=plan["core_problem"], height=80, key=f"cr_problem_{suffix}")
    main_message = st.text_area("메인 메시지", value=plan["main_message"], height=90, key=f"cr_message_{suffix}")
    cta = st.text_input("CTA", value=plan["cta"], key=f"cr_cta_{suffix}")
    ...
    for idx, (copy, image) in enumerate(zip(plan["slides"], plan["images"]), start=1):
        with st.container(border=True):
            st.markdown(f"#### {idx}장")
            slides.append(st.text_area("카피", value=copy, height=130, key=f"cr_slide_{idx}_{suffix}"))
            images.append(st.text_area("이미지 방향", value=image, height=110, key=f"cr_image_{idx}_{suffix}"))
...
def main():
    st.set_page_config(page_title="Cardnews Planner Copy Reviewed", page_icon="🧩", layout="wide")
    init_cardnews_table()

    st.title("🧩 카드뉴스 설계안 생성 Copy Reviewed")
    st.caption("문장 어색함을 줄이기 위해 프리셋과 생성 문장을 전면 검수한 버전입니다.")
    ...
    with i2:
        ...
        st.write(f"**문제 정의:** {intro['problem']}")

    c4, c5 = st.columns(2)
    with c4:
        brand = st.text_input("브랜드명", value="eaf:")
        target = select_with_custom("타깃 고객", TARGET_PRESETS, "cr_target", "브랜드/마케팅 담당자")
        cta = select_with_custom("CTA", CTA_PRESETS, "cr_cta_preset", "DM 포트폴리오/견적")
    with c5:
    ...
    p1, p2 = st.columns(2)
    with p1:
        insight = select_with_custom("벤치마크에서 가져올 인사이트 / 전개 구조", INSIGHT_PRESETS, "cr_insight", "목적-타깃-메시지-CTA", height=90)
        proof = select_with_custom("근거 / 설명", PROOF_PRESETS, "cr_proof", "수정 리스크 감소", height=90)
    with p2:
        solution = select_with_custom("브랜드 관점의 해결 방식", SOLUTION_PRESETS, "cr_solution", "전환 구조 먼저 설계", height=90)
        offer = select_with_custom("서비스 / 제안 문장", OFFER_PRESETS, "cr_offer", "브랜드/제품/유튜브 통합", height=90)
    ...
    plan = build_plan(ctx)
    signature = "|".join([str(int(row["id"])), template, tone, visual_style, intro_label, intro["hook"], intro["body"], brand, target, cta, angle, problem, insight, solution, proof, offer])
    ...
    with col_refresh:
        if st.button("현재 선택값으로 초안 다시 생성"):
            ...
            st.rerun()
    with col_info:
        st.info("드롭다운과 입력값이 바뀌면 아래 초안이 자동 갱신됩니다. 문장은 사람이 읽는 카드뉴스 호흡에 맞춰 짧게 정리됩니다.")

    st.markdown("---")
    draft = st.session_state.get("copy_reviewed_plan", plan)
    version = st.session_state.get("copy_reviewed_version", 0)
