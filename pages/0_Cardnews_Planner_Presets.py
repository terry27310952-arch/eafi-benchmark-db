...
def build_plan(ctx):
    ...
    if template == "문제폭로형":
        ...
        slides = [
            hookify("이건 편집 퀄리티 문제가 아닙니다", tone),
...
def update_draft_state(plan, signature, force=False):
    if force or st.session_state.get("preset_signature") != signature or "preset_plan" not in st.session_state:
        st.session_state["preset_signature"] = signature
        st.session_state["preset_plan"] = plan
...
def render_editable_plan(plan, version):
    suffix = f"v{version}"
    st.markdown("### 핵심 설계")
    title = st.text_input("저장할 주제명", value=plan["title"], key=f"preset_title_{suffix}")
    core_problem = st.text_area("핵심 문제", value=plan["core_problem"], height=80, key=f"preset_problem_{suffix}")
    main_message = st.text_area("메인 메시지", value=plan["main_message"], height=90, key=f"preset_message_{suffix}")
    cta = st.text_input("CTA", value=plan["cta"], key=f"preset_cta_{suffix}")
    ...
    for idx, (copy, image) in enumerate(zip(plan["slides"], plan["images"]), start=1):
        with st.container(border=True):
            st.markdown(f"#### {idx}장")
            slides.append(st.text_area("카피", value=copy, height=130, key=f"preset_slide_{idx}_{suffix}"))
            images.append(st.text_area("이미지 방향", value=image, height=110, key=f"preset_image_{idx}_{suffix}"))
...
def main():
    st.set_page_config(page_title="카드뉴스 설계안 생성 프리셋", page_icon="🧩", layout="wide")
    init_cardnews_table()

    st.title("🧩 카드뉴스 설계안 생성 프리셋")
    st.caption("핵심 입력값을 드롭다운으로 선택해 카드뉴스 초안을 빠르게 생성합니다.")
    ...
    with c5:
        ...
        target = select_with_custom("타깃 고객", TARGET_PRESETS, "target", "브랜드/마케팅 담당자")
        cta = select_with_custom("CTA", CTA_PRESETS, "cta", "DM 포트폴리오/견적")
    with c6:
        ...
        problem = st.text_area("핵심 문제", value=clean(row.get("hook_point"), "결과물 퀄리티만 보고 전환 구조를 놓치는 문제"), height=80)

    p1, p2 = st.columns(2)
    with p1:
        insight = select_with_custom("벤치마크에서 가져올 인사이트 / 전개 구조", INSIGHT_PRESETS, "insight", "목적-타깃-메시지-CTA", height=90)
        proof = select_with_custom("근거 / 설명", PROOF_PRESETS, "proof", "수정 리스크 감소", height=90)
    with p2:
        solution = select_with_custom("브랜드 관점의 해결 방식", SOLUTION_PRESETS, "solution", "전환 구조 먼저 설계", height=90)
        offer = select_with_custom("서비스 / 제안 문장", OFFER_PRESETS, "offer", "브랜드/제품/유튜브 통합", height=90)
    ...
    plan = build_plan(ctx)
    signature = "|".join([str(int(row["id"])), template, tone, visual_style, str(auto_review), brand, target, cta, angle, problem, insight, solution, proof, offer])
    ...
    with col_refresh:
        if st.button("현재 선택값으로 초안 다시 생성"):
            ...
            st.rerun()
    with col_info:
        st.info("드롭다운과 입력값이 바뀌면 아래 초안이 자동 갱신됩니다. 필요하면 직접 수정 후 저장하세요.")

    st.markdown("---")
    draft = st.session_state.get("preset_plan", plan)
    version = st.session_state.get("preset_version", 0)
